%% Calculates all observables for scenarios where bycatch is chosen randomly
...
parfor i4 =1:length(cutoff)
  %  Data = cell(1,400); %all data regarding variables for each network
   ...
   % cutoffb = cutoffvalues(i4,2);
    ...
    medianobservables = zeros(length(present),16); 
...
%mode
    ...
    j = sprintf('newmedian_survST_cutofft%d.mat',cutofft);
    var = cell(1,2);
