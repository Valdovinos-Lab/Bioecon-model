...
load('minbynetworkindex.mat')
%load("cutoffvalues.mat")
load("fishfreeavgbiomass")
seasonlength = 5*365+1;
kl = length(present);
cutoff = [0.15 0.3 0.45 0.6];
parfor i4 =1:length(cutoff)
  %  Data = cell(1,400); %all data regarding variables for each network
    ...
    for i=1:length(present)
        ...
        avgintarbybiomass = avgintarbybiomass(minbyind{i},:); %taking only the cases where the min bycatch is harvested
        j = sprintf('sharedtargetquota_k%d_Emax1_cutofft%d_cat2.000000e-03_cab2.000000e-03_costratio2.500000e-01_hill1.200000e+00_hsb2.000000e-01_fa4.000000e-01_fm4.000000e-01.mat',present(1,i),cutofft);
        a = load(j);
        a = a.var;
        a = a(minbyind{i}); %only selecting those cases where bycatch is min
     ...
     j = sprintf('new_median_MinBy_survST_cutofft%d.mat',cutofft);
    var = cell(1,2);
