...
function [Ap] = discreteeffortssharedtargetbycatchquota(Simhbdata,k,N,costratio,cat,cab,cutofft,cutoffb,nharvest,nbycatch,inE,nseasons,hill, hsb,fa,fm,target,bycatch)
...
%% SIMULATIONS -------------------------------------------------------------------------
    ...
    Bbmin = B0(bycatch)*cutoffb; %minimum levels of biomass of bycatch after which bycatch policy is permitted
    options=odeset('RelTol',10^-8,'AbsTol',10^-8);

    Bcutoffeff = (costratio./de)*(sum(B0(target)));   %respective target biomass after which effort would go to 0 for fishers 
    Bext = 10^(-6);
    for w=1:nseasons
        ...
        for days=1:365
            ...
            TAC = [B0(target)-Btmin,B0(bycatch)-Bbmin]; %difference in current biomass and minimum levels of target
    ...
    Ap = cell(1,2);
    Ap{1} =TS; %complete timeseries of all species biomass, fisher efforts, target and bycatch harvest
    Stats=zeros(1,2);
